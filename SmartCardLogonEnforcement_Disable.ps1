...
$AppName = "Disable_SmartCardLogon_Enforcement"
...
try
{
    ...
    Write-Log "Reg path HKLM:\SYSTEM\CurrentControlSet\Services\SCPolicySvc found, updating Reg key Start with value 3"
    New-ItemProperty -LiteralPath 'HKLM:\SYSTEM\CurrentControlSet\Services\SCPolicySvc' -Name 'Start' -Value 3 -Force -ErrorAction SilentlyContinue | Out-Null
    Write-Log "Reg key value updated"
}
catch 
{
...
Remove-Item -path C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\win32-Activate_SmartCardLogon_Enforcement.log -Force
